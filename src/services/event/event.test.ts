...
test("should check peer health for more than one", async () => {
  ...
  const { loadBalancers } = await NodesModel.findOne({ _id: "619299740f6e6600109d0192" });
  const status = await event.getBackendServerCount("bscmainnet", loadBalancers);
...
const mock = {
  ...
  type: "error",
  title: "[Triggered on {@conditions:NOT_SYNCHRONIZED}] HARMONY-2A/HRM",
  status: "",
