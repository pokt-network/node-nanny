...
import { HostsModel, LocationsModel, NodesModel, IHost } from "./models";
...
(async () => {
  ...
  for await (const host of hostsJSON) {
    ...
    if (location && !exists) {
      ...
      const { id: newId } = await HostsModel.findOne({ name: host.name });
      hostIdsMap[host._id.$oid] = new Types.ObjectId(newId);
    }
  }

  for await (const node of nodesJSON) {
    const exists = !!(await NodesModel.exists({ port: node.port, server: node.server }));
    if (!exists && hostIdsMap[node.host.$oid]) {
