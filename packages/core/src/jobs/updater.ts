...
(async () => {
  ...
  console.log('⛓️🔮 Initiating Chains & Oracles updater ...');
  ...
  console.log(
    `⛓️🔮 Fetching with latest chain ${latestChain} & latest oracle ${latestOracle} ...`,
  );
  const {
    data: { chains, oracles },
  ...
  if (chains?.length || oracles?.length) {
    ...
    console.log('⛓️🔮 No new chains or oracles found ...');
  }
